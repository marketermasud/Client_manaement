...
document.addEventListener("DOMContentLoaded", function () {
  MPAuth.ready(function () {
    MPDB.init(function () {
      ...
      document.getElementById("s_status").addEventListener("change", onServiceStatusChange);
...
// Start date stays manual. Whenever the person picks any status other than
// Active, the service is treated as finished, so End Date fills itself in
// with today's date if it isn't already set — no manual step needed.
// Switching back to Active clears it, since Active services are "Ongoing".
function onServiceStatusChange() {
  const status = document.getElementById("s_status").value;
  const endDateEl = document.getElementById("s_endDate");
  if (status === "Active") {
    endDateEl.value = "";
