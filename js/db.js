...
const MPDB = (function () {
  ...
  function emptyData() {
    // Full world currency list lives in js/currencies-data.js (WORLD_CURRENCIES),
  ...
  function clientOutstanding(clientId) {
    const rows = getPayments({ clientId }).filter(p => p.status !== "Paid");
    const byCode = {};
    rows.forEach(p => {
      const owed = paymentOutstanding(p);
      if (owed <= 0) return;
      byCode[p.currency] = byCode[p.currency] || { code: p.currency, amount: 0, bdt: 0 };
      byCode[p.currency].amount += owed;
      byCode[p.currency].bdt += owed * Number(p.exchangeRate || 1);
    });
  ...
  function totalOutstandingBDT() {
    return DATA.payments.filter(p => p.status !== "Paid")
      .reduce((s, p) => s + paymentOutstanding(p) * Number(p.exchangeRate || 1), 0);
  }
  ...
  return {
    ...
    pendingPayments, paymentOutstanding, clientOutstanding, totalOutstandingBDT,
